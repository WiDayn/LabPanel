...
func (s *SystemctlService) Restart() error {
	cmd := exec.Command("sudo", "systemctl", "restart", s.cfg.AppService)
...
func (s *SystemctlService) GetStatus() (bool, string, error) {
	cmd := exec.Command("systemctl", "is-active", s.cfg.AppService)
	output, err := cmd.CombinedOutput()
	if err != nil {
	...
	// 获取详细状态
	cmd = exec.Command("systemctl", "status", s.cfg.AppService, "--no-pager", "-l")
